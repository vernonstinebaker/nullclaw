...
Upstream `nullclaw/nullclaw` is dormant (no maintainer merges since 2026-04-17; 30 PRs open at assessment on 2026-09-23 — corrected from an earlier "28"; 26 open as of end of day after four closures below). This fork (`vernonstinebaker/nullclaw`) now maintains its own `main` directly. This scope incorporates selected **third-party** upstream PRs, one PR per increment, ranked against the fork's deployment environments: the OrangePi `webdav` MCP stdio server (`mcp_webdav_*` in an `always` tool-filter group), the bilingual `docs/en` + `docs/zh` tree, and the three deployment hosts (macOS arm64, OrangePi riscv64, Radxa aarch64 — daemon 24/7, Mattermost + Discord, sqlite memory, cron heartbeats).
...
| 2026-09-23 | Upstream activity after the fork's merged-into-fork comments: #979, #980, #969, #981 (all valonmulolli's) closed upstream today, unmerged. Net disposition of the 30 assessed PRs: 15 resolved by this fork's main (9 vernonstinebaker PRs + #985/#776/#777/#979/#984 intakes + #980 substance via #959); #969/#981 closed upstream but deliberately not taken; 13 third-party still open and parked (tier-3/4) | — |
